import dspy
from models.presentation import PresentationTitleSubtitleInput, PresentationTitleSubtitleOutput, \
    PresentationContentInput, \
    PresentationContentOutput, PresentationBulletPointsInput, PresentationBulletPointsOutput, \
    PresentationSpeakerNoteInput, \
    PresentationSpeakerNoteOutput, PresentationImageGenerationPromptInput, PresentationImageGenerationPromptOutput
...
class SpeakerNoteGeneratorSignature(dspy.Signature):
    ...
    speaker_note: str = dspy.OutputField(desc="Speaker notes for the slide, this is the content user will read out "
                                              ...
                                              "texts")
    summary: str = dspy.OutputField(desc="Summary of the slide content for context and coherence with previous slides. Complete the text with the main idea of the slide")
...
class ImageGenerationPromptGeneratorSignature(dspy.Signature):
    ...
    image_generation_prompt: str = dspy.OutputField(desc="Prompt for image generation aligned with the slide's content, "
